...
ssh -t $username'@'$serverIp "cd apps/mpba && tar -zxvf .tmpdeploy.tar.gz && rm .tmpdeploy.tar.gz && rsync -a .tmpdeploy/* . && rm -rf .tmpdeploy && npm install --production && pm2 reload server"
